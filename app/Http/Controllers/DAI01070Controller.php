...
ORDER BY
    ...
    public function Search($vm)
    {
        ...
        SELECT
            ...
            ,COU.コース名
            ,SHO.商品ＣＤ
            ,ISNULL(MOB_MOTI.個数,0) - (case ISNULL(MOB_HAN.実績入力フラグ,0) when 1 then ISNULL(MOB_HAN.販売数,0) else ISNULL(MOB_YOS.予測数,0)end) + ISNULL(MOB_IDO_UKE.個数,0) - ISNULL(MOB_IDO_WATASHI.個数,0) AS 個数
        FROM
            [コースマスタ] COU
                INNER JOIN [担当者マスタ] TAN ON
                    COU.担当者ＣＤ = TAN.担当者ＣＤ
                CROSS JOIN TAISHO_SHOHIN SHO
				LEFT OUTER JOIN MOBILE_YOSOKU MOB_YOS ON
