...
class DAI06040Controller extends Controller
{
    ...
    public function Search($vm)
    {
        ...
        $sql = "
			...
			from
				チケット残_SV
				INNER JOIN 得意先マスタ       on チケット残_SV.得意先ＣＤ = 得意先マスタ.得意先ＣＤ
				INNER JOIN 得意先コースマスタ on 得意先コースマスタ.得意先ＣＤ = チケット残_SV.得意先ＣＤ
				INNER JOIN 商品マスタ         on 商品マスタ.商品ＣＤ = チケット残_SV.商品ＣＤ
			where
				チケット残_SV.チケット残数 > 0 OR チケット残_SV.チケットSV > 0
			AND 得意先マスタ.部署CD=$BushoCd
			),
			抽出データ AS
			(
				SELECT
				...
				from
					...
					LEFT OUTER JOIN 部署マスタ BUSYO on
						URIAGE_MEISAI.部署ＣＤ = BUSYO.部署CD
					LEFT OUTER JOIN 得意先コースマスタ COU on
						URIAGE_MEISAI.得意先ＣＤ   = COU.得意先ＣＤ
					LEFT OUTER JOIN チケット発行得意先 TOKUISAKI on
				...
				AND	TOKUISAKI.得意先ＣＤ IS NOT NULL
				union
				select distinct
					 BUSYO.部署ＣＤ
					,BUSYO.部署名
			...
			SELECT
				IIF(D.得意先ＣＤ IS NULL, 2, 1) AS 処理区分,
				チケット残数.*
			FROM
				チケット残数
				LEFT OUTER JOIN (
