...
then 'ﾌ' end ふりかけ,
case
when tokui.味噌汁区分 = 1 then 'ﾐ' end みそしる
FROM
コーステーブル coutbl
left join
コースマスタ cou on cou.コースＣＤ = coutbl.コースＣＤ and cou.部署ＣＤ = coutbl.部署ＣＤ
left join 得意先マスタ tokui on coutbl.得意先ＣＤ = tokui.得意先ＣＤ and coutbl.部署ＣＤ = tokui.部署ＣＤ
left join 部署マスタ busyo on coutbl.部署ＣＤ = busyo.部署CD
left join 注文データ chumon on coutbl.得意先ＣＤ = chumon.得意先ＣＤ and coutbl.部署ＣＤ = chumon.部署ＣＤ and chumon.注文区分 = 0 AND CONVERT(varchar, chumon.注文日付, 112) = '20190903'AND chumon.現金個数 + chumon.掛売個数 > 0
where
coutbl.部署ＣＤ =".$BushoCd."
AND
".$BushoCd." <= coutbl.コースＣＤ
AND
coutbl.コースＣＤ <= ". $BushoCd."
AND
cou.コース区分 = ".$CourseKbn."
order by  coutbl.部署ＣＤ, coutbl.コースＣＤ, coutbl.ＳＥＱ
";
